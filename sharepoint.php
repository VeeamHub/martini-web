<?php
include(__DIR__ . '/core/veeam.vbo.class.php');
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="css/selfservicestyle.css" />
...
<nav class="navbar navbar-inverse navbar-static-top">
	...
	<ul class="nav navbar-nav navbar-right">
	  <li><a href="#"><span class="fa fa-user"></span> Welcome <i><?php echo $_SESSION['name']; ?></i> !</a></li>
...
$(document).on('click', '.btn-start-restore', function(e) {
	...
	if ($(this).data('latest')) {
		...
		if (!document.getElementById('pit-date').value) { /* No date has been selected */
			...
			Swal.fire({
				...
				text: 'No date selected, please select a date first before starting the restore or use the \"explore last backup\" button.'
			})
			return;
		} else {
    ...
    $.get('core/veeam.php', {'action' : 'startrestore', 'json' : json, 'id' : oid}).done(function(data) {
...
$(document).on('click', '.btn-stop-restore', function(e) {
	...
	swalWithBootstrapButtons.fire({
		...
		if (result.value) {
			$.get('core/veeam.php', {'action' : 'stoprestore', 'id' : rid}).done(function(data) {
				swalWithBootstrapButtons.fire({
					type: 'success', 
...
$(document).on("click", ".download-file", function(e) {
	...
	$.get("core/veeam.php", {"action" : "exportsharepointitem", "itemid" : itemid, "siteid" : siteid, "rid" : rid, "json" : json, "type" : filetype}).done(function(data) {
		e.preventDefault();
...
$(document).on("click", ".restore-original", function(e) {
	...
	swalWithBootstrapButtons.fire({
		...
		if (result.value) {
			...
			$.get("core/veeam.php", {"action" : act, "itemid" : itemid, "siteid" : siteid, "rid" : rid, "json" : json, "type" : filetype}).done(function(data) {
				Swal.fire({
					type: 'info',
					title: 'Item restore',
					text: '' + data
...
function loadFolderItems(folderid, parentid, rid, siteid) { /* Used for navigation to next folder */
    ...
    $.get("core/veeam.php", {"action" : "getsharepointitemsbyfolder", "folderid" : folderid, "rid" : rid, "siteid" : siteid, "type" : "folders"}).done(function(data) {
    ...
    $.get("core/veeam.php", {"action" : "getsharepointitemsbyfolder", "folderid" : folderid, "rid" : rid, "siteid" : siteid, "type" : "items"}).done(function(data) {
    ...
    $.get("core/veeam.php", {"action" : "getsharepointitemsbyfolder", "folderid" : folderid, "rid" : rid, "siteid" : siteid, "type" : "documents"}).done(function(data) {
        responsedocuments = JSON.parse(data);
    });
    <?php
    }
...
function loadParentFolderItems(parentid, rid, siteid) { /* Used for navigation to parent folder */
    ...
    /* First we load the folders */
    $.get("core/veeam.php", {"action" : "getsharepointitemsbyfolder", "folderid" : parentid, "rid" : rid, "siteid" : siteid, "type" : "folders"}).done(function(data) {
        responsefolders = JSON.parse(data);
    });

    /* Second we load the documents */
    $.get("core/veeam.php", {"action" : "getsharepointitemsbyfolder", "folderid" : parentid, "rid" : rid, "siteid" : siteid, "type" : "documents"}).done(function(data) {
        responsedocuments = JSON.parse(data);
    });
    ...
    setTimeout(function(e) {
        if (typeof responsefolders !== undefined && responsefolders.results.length != '0') {
            ...
            $.get("core/veeam.php", {"action" : "getsharepointparentfolder", "folderid" : newparentid, "rid" : rid, "siteid" : siteid, "type" : "folders"}).done(function(data) {
            ...
            $.get("core/veeam.php", {"action" : "getsharepointparentfolder", "folderid" : newparentid, "rid" : rid, "siteid" : siteid, "type" : "documents"}).done(function(data) {
                parentdata = JSON.parse(data);
...
function loadItems(folderid, siteid, rid, offset) { /* Used for loading additional items in a folder */
    ...
    $.get("core/veeam.php", {"action" : "getsharepointitems", "folderid" : folderid, "rid" : rid, "siteid" : siteid, "offset" : offset, "type" : "folders"}).done(function(data) {
        responsefolders = JSON.parse(data);
    });

    $.get("core/veeam.php", {"action" : "getsharepointitems", "folderid" : folderid, "rid" : rid, "siteid" : siteid, "offset" : offset, "type" : "documents"}).done(function(data) {
        responsedocuments = JSON.parse(data);
    });
